        ...
        for coin in COINS_LIST:
            ...
            if abs(score) >= 3 and ok and can_alert(coin):
                ...
                for chat_id in ALERT_CHAT_IDS:
                    await safe_send(chat_id=chat_id, text=msg)
    except Exception as e:
...
async def send_flow_alerts(context, coin: str, sig: dict):
    ...
    for chat in list(ALERT_CHAT_IDS):
        try:
            await safe_send(chat_id=chat, text=msg)
        except Exception as e:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        await safe_send(chat_id=chat_id, text=txt, reply_markup=coin_actions_markup(coin))
        ...
        if not inds:
            await safe_send(chat_id=chat_id, text="Không đủ dữ liệu.", reply_markup=coin_actions_markup(coin))
            return
        text = (f"📋 {coin} (1H):\n"
        ...
        await safe_send(chat_id=chat_id, text=text, reply_markup=coin_actions_markup(coin))
        ...
        await safe_send(chat_id=chat_id, text=ai_text, reply_markup=coin_actions_markup(coin))

    elif data == "back_coins":
        await safe_send(chat_id=chat_id, text="📊 Top Coins (select):", reply_markup=coins_page_markup(0))

    elif data == "toggle_alerts":
        if chat_id in ALERT_CHAT_IDS:
            ALERT_CHAT_IDS.remove(chat_id)
            await safe_send(chat_id=chat_id, text="⚡ Alerts: OFF", reply_markup=main_menu())
        else:
            ALERT_CHAT_IDS.add(chat_id)
            await safe_send(chat_id=chat_id, text="⚡ Alerts: ON", reply_markup=main_menu())
        ...
        await safe_send(chat_id=chat_id, text=news_text, reply_markup=news_menu_markup(coin))
    ...
    if data == "research_long":
        ...
        await safe_send(chat_id=chat_id, text=text)
    ...
    try:
        ...
        if LAST_NEWS_HOUR is None or (utcnow - LAST_NEWS_HOUR) >= NEWS_HOURLY_COOLDOWN:
            ...
            if new_articles:
                ...
                for chat in list(ALERT_CHAT_IDS):
                    try:
                        await safe_send(chat_id=chat, text=text)
                    except Exception:
...
async def research_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, mode="long"):
    ...
    await safe_send(chat_id=chat_id, text=f"🔎 Đang quét coins ({mode.upper()})...")
...
async def deepcoin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if not context.args:
        await safe_send(chat_id, "Ví dụ: /deepcoin BTC")
        return
    coin = context.args[0].upper()
    if not coin.endswith("-USDT") and not coin.endswith("-USD"):
        coin = coin + "-USDT"
    waiting_msg = await safe_send(chat_id, f"⏳ Đang phân tích sâu cho {coin}...")
